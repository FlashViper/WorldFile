...
func _get_importer_name() -> String:
	return "flashviper.worldeditor.importer_level"

func _get_visible_name() -> String:
	return "Level Importer"

func _get_recognized_extensions() -> PackedStringArray:
	return PackedStringArray(["level", "lvl"])
...
func _import(source_file: String, save_path: String, options: Dictionary, platform_variants: Array, gen_files: Array) -> Error:
	...
	var level := LevelFile.load_from_file(source_file)
	var filename = save_path + "." + _get_save_extension()
	
	var err := ResourceSaver.save(level, filename)
